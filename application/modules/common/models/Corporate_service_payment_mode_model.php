...
class corporate_service_payment_mode_model extends Base_Model
    ...
    public function findById($id, $deleted = false)
    {
        $this->db->select('id as corporate_service_payment_mode_id,
                           ...
                           deleted_at,
                           deleted_by');
        $this->db->from('iafb_holding_account.corporate_service_payment_mode');
        if(!$deleted)
        {
    ...
    public function findByParam(Product $config, $limit, $page)
    {
        $init_predicate = true;
        $offset = ($page - 1) * $limit;
        $this->db->start_cache(); //to cache active record query
        $this->db->select('id as product_id,
                           ...
                           is_inquirable,
                           max_total_fee,
                           service_provider_selling_price,
                           display_amount,
                           is_active,
                           created_at,
                           created_by,
                           ...
                           deleted_at,
                           deleted_by');
        $this->db->from('iafb_holding_account.product');
        $this->db->where('deleted_at', NULL);
        if($config->getCode()) {
            $this->db->where('code', $config->getCode());
        }
        if($config->getName()) {
            $this->db->like('name', $config->getName());
        }
        if($config->getBillerId()) {
            $this->db->where('biller_id', $config->getBillerId());
        }
        if($config->getIsActive()) {
            $this->db->where('is_active', $config->getIsActive());
        }
        if($config->getIsInquirable()) {
            $this->db->where('is_inquirable', $config->getIsInquirable());
        }
        if($config->getCorporateServiceId()) {
            $this->db->where('corporate_service_id', $config->getCorporateServiceId());
        }
        ...
        if( $query->num_rows() > 0)
        {
            return $this->mapCollection($query->result(), new ProductCollection(), $total);
        }
    ...
    public function insert(CorporateServicePaymentMode $payment_mode)
    {
        ...
        $this->db->set('role_id', $payment_mode->getRoleId());
        $this->db->set('created_by', $payment_mode->getCreatedBy());
        $this->db->set('created_at', IappsDateTime::now()->getUnix());

        if( $this->db->insert('iafb_holding_account.corporate_service_payment_mode') )
        {
            return true;
        }
    ...
    public function update(CorporateServicePaymentMode $payment_mode)
    {
        ...
        if( $this->db->update('iafb_holding_account.corporate_service_payment_mode') )
        {
            return true;
        }
